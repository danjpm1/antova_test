  ...
  return (
    <Link href={href}>
      <div className="group relative overflow-hidden rounded-xl cursor-pointer transition-transform duration-300 ease-out hover:scale-[1.02] aspect-[4/3]">
        <img src={image || "/placeholder.svg"} alt={alt} className="w-full h-full object-cover object-center" />
        <div
...
export default function AntovaBuilders() {
  ...
  const isSmartVisible = useScrollThreshold(smartRef, SMART_SECTION_THRESHOLD)
  ...
  const smartBgColor = isSmartVisible ? "bg-white" : "bg-black"
  const smartTextColor = isSmartVisible ? "text-black" : "text-white"

  return (
    <div className={`min-h-screen overflow-x-hidden ${bgColor} transition-colors duration-300 ease-in-out`}>
      <Navbar />
